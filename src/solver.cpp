...
public:
    ...
    bool operator < (const InputFitnessPair &rhs) const { //比较 lhs , rhs
...
private:
    void pushHeap(double input, double fitness) {
        ...
        std::push_heap(topInputsRandom.begin(), topInputsRandom.end(), pairGreater); // 最大堆
    }
    void popHeap() {
...
private:
    ...
    std::mt19937 mtGenerator; // 一种随机数生成器
    ...
    void _1RandomSearch() {
        ...
        for (int i = 1; i <= randomIteration; i++) {
            ...
            for (const auto &info : infoList) {
                ...
                double fit = fpUtil::revisedCondition(info.opcode, info.op1, info.op2); // 计算条件数, op1 opcode op2
...
int main(int argc, char *argv[]) {
    ...
    EvoSolver es;
    std::unique_ptr<FloatingPointFunction> funcPtr; // std::unique_ptr 是通过指针占有并管理另一对象，并在 unique_ptr 离开作用域时释放该对象的智能指针。
